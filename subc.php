...
mysqli_select_db($link,"hotels");

?>

<html>
<body>
<h3>BOOK THREE-STAR HOTELS HERE</h3>
<h3 class="head">BOOK FIVE-STAR HOTELS HERE</h3>
<form action="#" method="post">
            <select name="select3" class="select">
                <?php  
                $res=mysqli_query($link,"select * from MyGuests");
                while($row=mysqli_fetch_array($res))
                {if($row["hotel_types"]=="THREE-STAR"){
                    ?>
                <option ><?php
                
                 echo $row["addres"] ?></option>
                <?php
                }}
                ?>
...
<form  action="#" method="post">
            <select name="select5" class="select1">
                <?php  
                $res=mysqli_query($link,"select * from MyGuests");
                while($row=mysqli_fetch_array($res))
                {if($row["hotel_types"]=="FIVE-STAR"){
                    ?>
                <option ><?php
                echo $row["addres"] ?></option>
                <?php
...
<form action="#" method="post">
        ...
        <a href="page.html" class="home">HOME</a>

<?php

if(isset($_POST["contact"])){
    $val3=$_POST["select3"]." hotel";
...
if(isset($_POST["submit"])){
    echo "<br><br><br><p align='center'> <font color=green  size='6pt'>HOTEL REGISTRATION SUCCESFUL</font> </p>";
}
?>
</body>
